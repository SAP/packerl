...
from rl.nn.hmpn.abstract.abstract_message_passing_base import AbstractMessagePassingBase
from rl.nn.hmpn.homogeneous.homogeneous_message_passing_base import HomogeneousMessagePassingBase
...
def get_hmpn_from_graph(*, example_graph: Union[Data, HeteroData],
    ...
    """
    if hasattr(example_graph, "u"):
        in_global_features = example_graph.u.shape[1]
    else:
...
def get_hmpn(*,
    ...
    architecture = base_config.get("architecture", "mpn").lower()  # todo remove default
    if architecture == "gat":
        raise NotImplementedError
    elif architecture == "mpn":
        if isinstance(in_node_features, Dict):
            ...
            base = HomogeneousMessagePassingBase(**params, node_name=node_name)
        else:
