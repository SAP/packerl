"""
Implementation of the actor class used in MAGNNETO.
"""
...
class Actor(tf.keras.Model):
    def __init__(self, 
        ...
        self.kernel_regularizer = None #tf.keras.regularizers.l2(0.01)
        #tf.keras.initializers.VarianceScaling(scale=1.0 / np.sqrt(3.0),mode='fan_in', distribution='uniform')
    ...
    def define_network(self):
        # message
        self.create_message = tf.keras.models.Sequential(name='create_message')
        self.create_message.add(tf.keras.layers.Dense(self.message_hidden_layer_size,
                kernel_initializer=self.hidden_layer_initializer, activation=self.activation_fn))
        self.create_message.add(tf.keras.layers.Dense(self.link_state_size,
                kernel_initializer=self.hidden_layer_initializer, activation=self.activation_fn))

        # link update
        self.link_update = tf.keras.models.Sequential(name='link_update')
        self.link_update.add(tf.keras.layers.Dense(self.first_hidden_layer_size,
                kernel_initializer=self.hidden_layer_initializer, activation=self.activation_fn))
        self.link_update.add(tf.keras.layers.Dense(self.final_hidden_layer_size,
                kernel_initializer=self.hidden_layer_initializer, activation=self.activation_fn))
        self.link_update.add(tf.keras.layers.Dense(self.link_state_size,
                kernel_initializer=self.hidden_layer_initializer, activation=self.activation_fn))

        # readout
        self.readout = tf.keras.models.Sequential(name='readout')
        self.readout.add(tf.keras.layers.Dense(self.first_hidden_layer_size, kernel_initializer=self.hidden_layer_initializer, 
                                kernel_regularizer=self.kernel_regularizer, activation=self.activation_fn))
        self.readout.add(tf.keras.layers.Dropout(self.dropout_rate))
        self.readout.add(tf.keras.layers.Dense(self.final_hidden_layer_size, kernel_initializer=self.hidden_layer_initializer, 
                                kernel_regularizer=self.kernel_regularizer, activation=self.activation_fn))
        self.readout.add(tf.keras.layers.Dropout(self.dropout_rate))
        self.readout.add(tf.keras.layers.Dense(self.num_actions, kernel_initializer=self.final_layer_initializer, 
